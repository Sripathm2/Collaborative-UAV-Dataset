#!/usr/bin/env python3
"""
build_ts_csv.py  (v2: direction-aware + optional config embedding)

Walks server_*/ folders containing raw flows-*.txt + attack_details_*.txt and
emits UAV-CAS_ts.csv with three list columns:

    packet_time, packet_size, packet_dir, Label
...
(e.g.  ddos=1000+replay=50,200,10,5,inc-20-2-image-logdist-adaptive-random-30-95).
Neither piece contains '|'.

Backward compat: if flows-*.txt has 3-tuples (legacy v1 data), all packets are
treated as dir=0 and a one-time warning is printed.
...
def main():
    ...
    ap.add_argument('--include_flags', action='store_true',
                    help='Add a third list column packet_flag (hex TCP flag '
                         'strings per packet) to the output. Header becomes '
                         'packet_time,packet_size,packet_flag,packet_dir,Label.')
    ...
    # header depends on --include_flags
    if args.include_flags:
        header = ['packet_time', 'packet_size', 'packet_flag', 'packet_dir', 'Label']
    else:
        header = ['packet_time', 'packet_size', 'packet_dir', 'Label']
